...
def lend_cur(active_cur, total_lended, lending_balances):
    ...
    while i < len(orders['amounts']):  # Iterate through prepped orders and create them if they work
        ...
        if hide_coins and below_min:
            log.log("Not lending {:s} due to the current rate {:.4f}% being below the minimum rate of {:.4f}%".format(active_cur,Decimal(orders['rates'][i]*100),(cur_min_daily_rate * 100)))
            return 0
        elif below_min:
